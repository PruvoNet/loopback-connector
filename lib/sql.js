...
SQLConnector.prototype.searchForPropertyInActual =
...
SQLConnector.prototype._buildWhere = function(model, where) {
  ...
  for (const key in where) {
    ...
    const p = props[key];
    if (p == null) {
      throw new Error('Unknown property ' + key + ' used for model ' + model);
    }
...
SQLConnector.prototype.buildOrderBy = function(model, order) {
  ...
  for (let i = 0, n = order.length; i < n; i++) {
    ...
    const key = t[0];
    if (!props[key]) {
      throw new Error('Unknown property ' + key + ' is used for model ' + model);
    }
    if (t.length === 1) {
